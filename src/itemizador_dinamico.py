...
def _calcular_bota_adaptativa(campeon: str, enemigos: list, botas_data: dict = None) -> tuple:
    ...
    if aa_heavy or (ad_dom and sum(1 for e in enemigos if e in _CRIT_CHAMPS) >= 2):
        return (
            BOTA_STEELCAPS,
            f"Placas de Acero — {int(aa_score)}/{n} enemigos dependen de autoataques. Reduce daño físico el 12%.",
            False,
        )
    if cc_heavy:
        return (
            BOTA_MERCURY,
            f"Botas de Mercurio — {int(total_cc)} pts de CC. Tenacidad reduce su duración el 30%.",
            False,
        )
    if ad_dom and ad_score >= 3:
        return (BOTA_STEELCAPS, f"Placas de Acero — {int(ad_score)}/{n} campeones hacen daño físico.", False)
    if ap_dom and total_cc >= 6:
        return (BOTA_MERCURY, f"Botas de Mercurio — {int(ap_score)} amenazas AP con CC. RM + tenacidad.", False)
    if total_cc >= 7:
        return (BOTA_MERCURY, f"Botas de Mercurio — {int(total_cc)} pts de CC. Tenacidad invaluable.", False)
    if ad_score >= 3 and aa_score >= 3:
        return (
            BOTA_STEELCAPS,
            f"Placas de Acero — {int(ad_score)} AD y {int(aa_score)} AA. Mejor valor defensivo.",
            False,
        )
    if slow_count >= 3:
        return (
            BOTA_SWIFTNESS,
            f"Botas de Rapidez — {int(slow_count)} enemigos con slows. Reduce ralentizaciones el 25%.",
            False,
        )
    if ap_dom:
        return (BOTA_MERCURY, "Botas de Mercurio — comp AP dominante. RM + tenacidad.", False)
    if botas_data:
...
def recomendar_items_situacionales(campeon: str, carril: str, enemigos: list) -> list:
    ...
    def _add(item_id: str, razon: str, categoria: str, prioridad: int = 2):
        sugs.append({"id": item_id, "razon": razon, "categoria": categoria, "prioridad": prioridad})

    # ── Conteos clave ─────────────────────────────────────────
    healers = sum(1 for e in enemigos if _norm(e) in {_norm(h) for h in _HEALING_CHAMPS})
    sum(1 for e in enemigos if e in _SHIELD_CHAMPS)
    ...
    if supresses >= 1:
        ...
        if soy_tirador or soy_asesino or soy_mago:
            ...
            _add(MIKAELS, f"Suppress de {nombres_sup} — Bendición de Mikael limpia el CC de tu aliado", "anti_cc", 1)
    elif cc_total >= 12 and not soy_tanque:
        _add(QSS, f"{cc_total} pts de CC enemigo — Quitarrunas limpia cualquier debuff y permite escapar", "anti_cc", 2)
    ...
    if soy_tanque and (crit_cnt >= 2 or ad_count >= 3):
        _add(
            GARGOYLE, "Comp AD pesada — Gárgola Pétrea: activa para duplicar blindaje en teamfight", "supervivencia", 3
        )
