...
def analizar_personalidad(historial_games: list) -> dict:
    ...
    if avg_k >= 8 and avg_d >= 5:
        ...
        estilo = "AGRESIVO"
        perfil = (
            "Jugador agresivo con mentalidad de carry. Busca outplays constantes y genera ventaja desde early game."
        )
    else:
        estilo = "BALANCEADO"
...
def detectar_habitos(historial_games: list) -> list:
    ...
    if post_loss_total >= 3:
        ...
        if wr_post_loss < wr_general - 10:
            insights.append(
                f"⚠️ Tu winrate cae de {wr_general}% a {wr_post_loss}% después de una derrota. Considera tomar un descanso tras perder."
            )
        elif wr_post_loss > wr_general + 5:
            insights.append(
                f"💪 Te recuperas bien tras derrotas: WR post-derrota = {wr_post_loss}% (vs {wr_general}% general)."
            )
    ...
    if ahead_total >= 4:
        ...
        if wr_ahead >= 70:
            insights.append(
                f"✅ Excelente jugando desde ventaja: cierras el {wr_ahead}% de partidas donde vas positivo en kills."
            )
        elif wr_ahead < 55:
            insights.append(
                f"⚠️ Te cuesta cerrar partidas: solo ganas el {wr_ahead}% cuando vas positivo en KDA. Trabaja en el macro y objetivos."
            )
    ...
    if len(cs_values) >= 3:
        ...
        if cs_early and cs_late:
            ...
            if drop > 1.5:
                insights.append(
                    f"📉 Tu CS baja de {avg_early}/min (early) a {avg_late}/min (late). Enfócate en mantener el farm en mid-late game."
                )

        if avg_cs_min < 5:
            insights.append(
                f"🎯 CS promedio bajo: {avg_cs_min}/min. Practica last-hitting en practice tool 10 min al día."
            )
        elif avg_cs_min > 7.5:
    ...
    if avg_deaths > 6:
        insights.append(
            f"🛡️ Promedio de muertes alto: {avg_deaths}/partida. Revisa tu posicionamiento y visión en el mapa."
        )

    # ─── Max derrotas seguidas ───
    if max_derrotas >= 4:
        insights.append(
            f"🔴 Rachas de hasta {max_derrotas} derrotas seguidas detectadas. Programa descansos cada 2-3 partidas."
        )
...
def generar_objetivos_semanales(historial_games: list) -> list:
    ...
    if avg_d > 5:
        issues.append(
            (
                "muertes",
                avg_d,
                f"🎯 Objetivo: Morir menos de 5 veces promedio (actual: {avg_d}). Enfócate en posicionamiento y wardear antes de pushear.",
            )
        )
    ...
    if avg_cs_min > 0 and avg_cs_min < 6:
        issues.append(
            (
                "cs",
                avg_cs_min,
                f"🎯 Objetivo: Alcanzar 6.5+ CS/min (actual: {avg_cs_min}). Dedica 10 min diarios al practice tool.",
            )
        )
    ...
    if avg_k < 3 and avg_d < 4:
        issues.append(
            (
                "impacto",
                avg_k,
                f"🎯 Objetivo: Aumentar tu impacto en teamfights. Busca participar en al menos 2 kills extra por partida (actual: {avg_k} kills).",
            )
        )
    ...
    if wr < 45:
        issues.append(
            (
                "wr",
                wr,
                f"🎯 Objetivo: Subir tu winrate a 50%+. Revisa tus counters y limita tu pool a 2-3 campeones principales (WR actual: {wr}%).",
            )
        )
    ...
    if avg_vis > 0 and avg_vis < 0.5:
        issues.append(
            (
                "vision",
                avg_vis,
                f"🎯 Objetivo: Mejorar visión a 0.8+/min (actual: {avg_vis}). Compra más pinks y usa el trinket en cooldown.",
            )
        )
    ...
    if avg_a < 3 and avg_k > 5:
        issues.append(
            (
                "equipo",
                avg_a,
                f"🎯 Objetivo: Jugar más para el equipo. Promedio de asistencias bajo ({avg_a}) pese a buen KDA. Rotea más.",
            )
        )

    # Tomar los 3 más críticos (ordenados por severidad)
    issues.sort(key=lambda x: {"muertes": 0, "wr": 1, "cs": 2, "vision": 3, "impacto": 4, "equipo": 5}.get(x[0], 99))
    ...
    while len(objetivos) < 3:
        genericos = [
            ...
            "🎯 Objetivo: Expandir tu pool de campeones en 1-2 picks nuevos esta semana.",
            "🎯 Objetivo: Ver 1 replay propio por día para identificar errores de posicionamiento.",
        ]
        for g in genericos:
