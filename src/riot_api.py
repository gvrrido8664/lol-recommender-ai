...
def actualizar_datos_riot():
    ...
    try:
        ...
        for _spell_id, details in data_spells.items():
            diccionario_hechizos[str(details["key"])] = {
                "nombre": details["name"],
                "icono": details["image"]["full"],
                "descripcion": limpiar_html(details.get("description", "")),
            }
        with open(CACHE_SPELLS, "w", encoding="utf-8") as f:
            json.dump(diccionario_hechizos, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"❌ Error descargando hechizos: {e}")
...
def cargar_runas():
    if not os.path.exists(CACHE_RUNAS):
        actualizar_datos_riot()
    with open(CACHE_RUNAS, "r", encoding="utf-8") as f:
        return json.load(f)


def cargar_mapeo_ids():
    if not os.path.exists(CACHE_IDS):
        actualizar_datos_riot()
...
def cargar_hechizos():
    if not os.path.exists(CACHE_SPELLS):
        actualizar_datos_riot()
    with open(CACHE_SPELLS, "r", encoding="utf-8") as f:
        return json.load(f)
