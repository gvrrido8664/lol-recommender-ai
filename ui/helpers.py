...
def _jungla_estilo(champ_name: str) -> dict:
    ...
    for _estilo, data in JUNGLA_ESTILO.items():
...
def sugerir_pathing_jungla(mi_champ: str, enemy_jungler: str, aliados: list, enemigos: list) -> dict:
    ...
    if enemy_jungler:
        ...
        if emy.get("label", "").startswith("⚡"):
            vs_tip = (
                f"⚠️ {enemy_jungler} es agresivo temprano — wards en entradas de jungla y juega seguro en nivel 1-3."
            )
...
def ajustar_shards_adaptativos(ids_runas: list, mi_champ: str, enemigo_lane: str, picks_en: list) -> list:
    ...
    from src.tags_champions import es_mago, es_tirador, obtener_dano, obtener_nivel_cc
...
MATCHUP_TIPS = {
    "Zed": [
        "Guarda tu dash/CC para despues de su R — actua cuando sale del shadow.",
    ...
    "Darius": [
        "No tradees cuando falle su Q — el borde cura y recarga rapido.",
    ...
    "Yasuo": [
        "Los champions con mucho poke pasan el early; evita caminar hacia el.",
    ...
    "Yone": [
        "Su R te ancla al suelo — usa flash antes de que caiga, no despues.",
        "Cuando entra en soul form pierde rango; kitele en esas ventanas.",
        "Nivel 6 es su pico; juega defensivo sin ult propio.",
    ],
    "Lux": [
        "Predice su Q en linea recta; los side-steps eliminan el 80% de su dano.",
    ...
    "Thresh": [
        "Su Q (gancho) tiene 2 partes — dodge la primera con lateral.",
    ...
    "Blitzcrank": [
        "Camina detras de minions en el carril — bloquean el gancho.",
    ...
    "Zac": [
        "Sus blobs (e-s) en el suelo le curan; destruyelos pisandolos antes.",
        "Su E (salto) anuncia antes de saltar — escuchalo y escapa lateral.",
        "Nivel 3 y 6 son sus picos de gank; wardea trinomio.",
    ],
    "Irelia": [
        "No la dejes stackear pasiva — ataca minions para resetearla.",
    ...
    "Nasus": [
        "Ralentizale el farmeo de Q con poke constante los primeros 10 min.",
    ...
    "Tryndamere": [
        "Su R dura 5s — CC y corre, no lo termines dentro de ult.",
        "Nivel 11 (ult 2 veces) es su pico — juega cerca de tower.",
        "Su furia (barrita) aumenta su CR; no lo dejes acumular sin pegarle.",
    ],
    "Fiora": [
        "Sus Vitals cambian de lado 4 veces; aprende su patron para quitarlos.",
        "Su R requiere golpear los 4 vitals — bordalo con cuerpo a cuerpo.",
    ],
    "Camille": [
        "Su E (gancho + dash) tiene un gap de 0.75s entre las dos partes — evade el segundo.",
        "Su R la encierra contigo sola — flash por la pared antes de que caiga.",
    ],
    "Renekton": [
        "Matchup de rabia: cuando tenga barra llena (50+) no tradees.",
        "Su W (combo de aturdimiento) requiere un hit previo — alejate cuando se active.",
    ],
    "Gangplank": [
        "Sus barriles tienen 2 cargas por defecto — elimina el primero antes de que encadene.",
        "Su E le quita CC activos — úsalos durante su ult para el minimo dano.",
    ],
    "Malphite": [
        "Full AP Malphite: tratalo como un mago disfrazado de tanque.",
        "Su R tiene CC de area — no estes agrupado contra el en teamfights.",
    ],
    "Morgana": [
        "Su W tiene un delay de 0.5s — side-step apenas lo empiece a lanzar.",
        "Su escudo (E) bloquea CCs; no gastes tu CC hard mientras este activo.",
    ],
    "Leona": [
        "No pelear cuando su Eclipse (W) esta activo — dura 3s.",
        "Su Q (stun) requiere que su E haya llegado primero — ve el orden.",
    ],
    "Nautilus": [
        "Su gancho Q marca — si falla, tiene un gran cooldown.",
        "Su auto-ataque ancla (pasiva) — no autoataquees en trades cortos si son malos.",
    ],
    "Jinx": [
        "Sin cargas de pasiva es un objetivo facil — matala antes de que empiece a resetear.",
        "Sus minas (E) duran 5s en el suelo — camina por los lados de la wave.",
    ],
    "Caitlyn": [
        "Sus trampas (W) se colocan tras stuns — ward los arbustos y no pases sobre ellas.",
        "Su ult puede ser bloqueada por un aliado entre tu y ella.",
    ],
    "Ezreal": [
        "Su E es su unico escape — forzalo con CC y luego all-in.",
        "Su Q tiene rango muy largo — no hagas linea recta en poke wars.",
    ],
    "Orianna": [
        "La pelota esta siempre en algun lugar — saber donde es el 80% del matchup.",
        "Su R en pelota lanzada — alejarse del centro es la defensa mas efectiva.",
    ],
    "Syndra": [
        "Sus bolas permanecen en el mapa — su R hace mas dano cuantas mas tenga.",
        "Su E aturde si una bola esta detras del target — no te pongas entre bolas y ella.",
    ],
    "Ahri": [
        "Puede hacer charm en W; el Q pasa dos veces — la vuelta hace mas dano.",
        "Sin R (3 dashes) es muy vulnerable — countergankea post ult.",
    ],
    "Leblanc": [
        "Su W deja un espejo — si no pone puntos en W empuja, si no regresa.",
        "Con silencio le cortas la cadena de burst — úsalo antes de su Q.",
    ],
    "Katarina": [
        "Sus daggers en el suelo la resetean — evita quedarte cerca de ellas.",
        "CC en el momento del dash la interrumpe completamente.",
    ],
    "Akali": [
        "Su campo de humo (W) la hace invisible — usa AoE para forzarla a salir.",
        "Su R primer dash no hace dano — el segundo si; flash tras el primero.",
    ],
    "Qiyana": [
        "Sus elementos cambian su kit — el Q con rio aturde, con arbol hace dano extra.",
        "Su R explota con colision de estructuras y rios; cuida los bordes del mapa.",
    ],
    "Rengar": [
        "Ward los arbustos antes de que llegue al 6; predice de que arbusto sale.",
        "Su pasiva (stack) aumenta su kit — hazle usar habilidades sin stacks completos.",
    ],
    "Evelynn": [
        "Invisible post-6 salvo anti-invis — itemiza Oraculo de la Trampa (pink ward).",
        "Su alurt aturde si la segunda parte impacta — sal del area con flash.",
    ],
    "Shaco": [
        "Sus cajas (W) se activan por aparicion repentina — no entres en arbustos sin ward.",
        "Puede clonar con R — el real tiene HP diferente en el numero del score.",
    ],
    "Nidalee": [
        "Sus javelinas hacen mas dano a distancia — angulate para reducir la distancia.",
        "En forma de puma su Q cura — activala cuando tenga vida baja.",
    ],
    "Hecarim": [
        "Su fantasma de invocador sube su movespeed para E — no lo persiga en linea recta.",
        "Su R tiene fear de area — agrupate lejos de walls para evitar el empuje.",
    ],
    "Warwick": [
        "Su W (rastrear heridos) se activa en <50% HP — juega conservador en esa zona.",
