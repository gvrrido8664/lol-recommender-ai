...
class PerfilTabMixin:
    ...
    def _riot_convert_match(self, raw: dict, my_puuid: str = ""):
        ...
        return {
            ...
            "gameCreationDate": datetime.fromtimestamp(game_creation_ts).strftime("%b %d, %Y %I:%M:%S %p")
            if game_creation_ts
            else "",
        ...
        with ThreadPoolExecutor(max_workers=3) as pool:
            ...
            for future in as_completed(futures):
                ...
                if downloaded - last_emit >= BATCH_EMIT:
                    ...
                    print(
                        f"[RiotAPI] {total}/{len(match_ids)} ({(total / len(match_ids) * 100):.0f}%) +{len(batch)} en {elapsed:.0f}s"
                    )
    ...
    def _riot_season_background(self, puuid: str, all_games: list, game_name: str, tag_line: str):
        ...
        try:
            ...
            if cached:
                ...
                nuevos_cache = [
                    g for g in cached if self._gid_or_fallback(g) and self._gid_or_fallback(g) not in existing_gids
                ]
                if nuevos_cache:
            ...
            if riot_ids:
                ...
                if not riot_games:
                    self.riot_error.emit(
                        "No se pudieron descargar partidas. Puede que tu API key haya expirado o tenga rate limit."
                    )
            else:
    ...
    def _fetch_perfil(self):
        ...
        try:
            ...
            if puuid:
                ...
                threading.Thread(
                    target=self._riot_season_background, args=(puuid, [], game_name, tag_line), daemon=True
                ).start()
    ...
    def _on_perfil_listo(self, data):
        ...
        try:
            ...
            # --- Nombre y nivel ---
            display_name = (
                perfil.get("displayName")
                or perfil.get("gameName")
                or perfil.get("summonerName")
                or perfil.get("name")
                or "Invocador"
            )
            ...
            if ruta_icon:
                self.lbl_prof_icon.setPixmap(
                    QPixmap(ruta_icon).scaled(60, 60, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                )
            ...
            try:
                if ranked_solo and ranked_solo.get("tier"):
                    registrar_lp(
                        ranked_solo["tier"],
                        ranked_solo.get("division", ""),
                        ranked_solo.get("lp", 0),
                        ranked_solo.get("wins", 0),
                        ranked_solo.get("losses", 0),
                        "RANKED_SOLO_5x5",
                    )
                if ranked_flex and ranked_flex.get("tier"):
                    registrar_lp(
                        ranked_flex["tier"],
                        ranked_flex.get("division", ""),
                        ranked_flex.get("lp", 0),
                        ranked_flex.get("wins", 0),
                        ranked_flex.get("losses", 0),
                        "RANKED_FLEX_SR",
                    )
    ...
    def _recalc_stats_cards(self):
        ...
        if total_games > 0:
            ...
            if champ_stats:
                ...
                if best_wr_champs:
                    best_champ = max(
                        best_wr_champs, key=lambda c: best_wr_champs[c]["wins"] / best_wr_champs[c]["games"]
                    )
    ...
    def _renderizar_historial(self, games):
        ...
        try:
            games = sorted(games, key=lambda g: self._parse_game_date(g) or datetime(2000, 1, 1), reverse=True)
        except Exception:
            pass
        ...
        sorted(set(self._clasificar_modo_juego(g) for g in self.historial_games))
    ...
    def _cargar_mas_partidas_scroll(self):
        ...
        try:
            ...
            really_new = [
                g for g in nuevas if self._gid_or_fallback(g) and self._gid_or_fallback(g) not in existing_ids
            ]
    ...
    def filtrar_historial(self, _=None):
        ...
        try:
            juegos = sorted(
                self.historial_games, key=lambda g: self._parse_game_date(g) or datetime(2000, 1, 1), reverse=True
            )
        except Exception:
