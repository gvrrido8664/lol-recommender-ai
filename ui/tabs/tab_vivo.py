...
class VivoTabMixin:
    ...
    def armar_tab_vivo(self):
        ...
        self.lbl_radar_tip = QLabel(
            "💡 <b>Consejo:</b> En Champ Select, prioriza counter-pickear a tu rival de línea. Revisa runas y hechizos recomendados abajo."
        )
        self.lbl_radar_tip.setWordWrap(True)
        self.lbl_radar_tip.setTextFormat(Qt.RichText)
        self.lbl_radar_tip.setStyleSheet(
            f"color: {TEXT_SECONDARY}; font-size: 10px; padding: 6px 10px; background-color: {BG_DARK}; border: 1px solid {BG_CARD_HOVER}; border-left: 3px solid {ACCENT_TEAL}; border-radius: 4px; margin-bottom: 2px;"
        )
        ...
        self.lbl_pathing_estilo.setStyleSheet("font-size: 12px; font-weight: bold; padding: 2px 0;")
    ...
    def auto_detectar_lcu(self):
        ...
        if fase != self._last_fase:
            ...
            if (
                self.user_settings.get("notificaciones_escritorio", True)
            ...
            try:
                if fase in ("GameStart", "InProgress"):
                    actualizar_discord_rpc(
                        details="En partida", state="League of Legends - SoloQ", large_text="Jugando"
                    )
                elif fase == "ChampSelect":
                    actualizar_discord_rpc(
                        details="Seleccion de campeones", state="Champ Select", large_text="Draftero"
                    )
                elif fase == "ReadyCheck":
                    actualizar_discord_rpc(details="Partida encontrada", state="Aceptando...", large_text="En cola")
                else:
                    actualizar_discord_rpc(
    ...
    def procesar_nombre_champ(self, cid, intent):
        ...
        if final_id != "0":
            return (
                "Wukong"
                if MAPEO_IDS_CAMPEONES.get(final_id) == "MonkeyKing"
                else MAPEO_IDS_CAMPEONES.get(final_id, "Desconocido")
            )
        return None
    ...
    def _radar_db_compute(self, draft):
        ...
        if not enemigo_lane and enemigos_procesados:
            ...
            for champ, _pos, _idx in enemigos_procesados:
                try:
            ...
            if not enemigo_lane:
                ...
                if not enemigo_lane:
                    for champ, pos, _idx in enemigos_procesados:
        ...
        if cache_rs is None:
            cache_rs = self._cache_runas_radar = {}
        cache_bans = getattr(self, "_cache_bans_radar", None)
        if cache_bans is None:
            cache_bans = self._cache_bans_radar = {}
        ...
        def _wr_task():
            if len(picks_al_db) >= 5 and len(picks_en_db) >= 5:
                ...
                if mi_campeon_db:
                    try:
                        bans_act = [
                            self.procesar_nombre_champ(b.get("championId", 0), 0)
                            for b in draft.get("bans", {}).get("myBans", [])
                        ]
        ...
        def _items_task():
            ...
            i_sit = (
                obtener_items_situacionales(mi_campeon_db, rol_api, enemigos_db_list, excluir=i_core)
                if picks_en_db
                else []
            )
        ...
        with ThreadPoolExecutor(max_workers=len(tasks)) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    key = result[0]
                    if key == "wr":
                        db_data["wr_5v5"] = result[1]
                        db_data["draft_id"] = result[2]
                    elif key == "bans":
                        ...
                        db_data["runas"] = result[1]
                        db_data["hechizos"] = result[2]
                    elif key == "items":
        ...
        try:
            ...
            if not enemigo_lane and enemigos_procesados:
                if enemigo_lane_db_pre:
                    ...
                    if not enemigo_lane:
                        mi_idx = next(
                            (i for i, j in enumerate(draft.get("myTeam", [])) if j.get("cellId") == mi_celda), -1
                        )
                        if 0 <= mi_idx < len(enemigos_procesados):
                            champ_idx = enemigos_procesados[mi_idx][0]
            ...
            if picks_al != self.last_aliados or picks_en != self.last_enemigos:
                ...
                if enemigo_lane != self.last_enemigo_lane:
                    self._actualizar_counters_vivo(
                        rol_api, enemigo_lane, counters_pre=db_data.get("counters") if db_data else None
                    )
                ...
                if len(picks_al_db) == 5 and len(picks_en_db) == 5:
                    ...
                    tendencia = (
                        "↑ Ventaja de Sinergia"
                        if wr > 52
                        else "↓ Desventaja de Draft"
                        if wr < 48
                        else "≈ Matchup Equilibrado"
                    )
                    ...
                    if mi_campeon:
                        if draft_id_pre is not None:
                            ...
                            try:
                                ...
                                self._draft_id_actual = guardar_draft(
                                    mi_campeon, rol_api, bans_actuales, picks_al, picks_en, wr
                                )
                            except Exception as e:
            ...
            if mi_campeon != self.last_my_champ or rol_api != self.last_my_role:
                ...
                bans_filtrados = [
                    (b, wr, p)
                    for b, wr, p in bans_sugeridos
                    if b not in self.last_aliados and b not in self.last_enemigos
                ][:4]
                if bans_filtrados:
                    for i, (ban, wr, partidas) in enumerate(bans_filtrados):
                        self.renderizar_icono(
                            ban,
                            "champ",
                            self.fr_bans_icons_vivo,
                            0,
                            i,
                            f"🚫 Baneo sugerido: {self._nombre_display(ban)}\n📊 WR rival: {wr}% en {partidas} partidas",
                            size=35,
                        )
                else:
            ...
            if enemigo_lane:
                ...
                if tips:
                    ...
                    self.lbl_matchup_tips.setText(f"🗡️ <b>vs {self._nombre_display(enemigo_lane)}:</b>  {tips_html}")
